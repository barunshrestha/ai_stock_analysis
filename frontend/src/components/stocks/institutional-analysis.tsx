...
import { Briefcase, ChevronDown, Loader2, RefreshCw } from "lucide-react";
...
function cacheLookupToAnalysis(data: InstitutionalCacheLookup): InstitutionalAnalysis | null {
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        {s?.stance && (
          <Badge variant="outline" className="capitalize">Stance: {s.stance}</Badge>
        )}
        {s?.confidence && (
          <Badge variant="secondary" className="capitalize">Confidence: {s.confidence}</Badge>
        )}
        {s?.catalysts?.slice(0, 3).map((c, i) => (
...
export function InstitutionalPerspectivePanel({ symbol }: { symbol: string }) {
      ...
      <CardContent>
        {!displayed && !generateMut.isPending && !generateMut.isError && (
          <div className="space-y-2 text-sm text-muted-foreground">
            ...
            <p>Hedge-fund PM style view: why institutions might buy or avoid, catalysts, and thesis — educational only.</p>
