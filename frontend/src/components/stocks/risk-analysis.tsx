  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        {s?.overall_risk && (
          <Badge variant="outline" className="capitalize">Overall: {s.overall_risk}</Badge>
        )}
        {s?.confidence && (
          <Badge variant="secondary" className="capitalize">Confidence: {s.confidence}</Badge>
        )}
        {s?.ranked_risks?.map((r) => (
          <Badge key={r.rank} variant="outline" className="capitalize">
            #{r.rank} {r.category}: {r.title}
          </Badge>
        ))}
        {data.updated_at && (
      ...
      <CardContent>
        {!displayed && !generateMut.isPending && !generateMut.isError && (
          <div className="space-y-2 text-sm text-muted-foreground">
            ...
            <p>Rank economic, disruption, competition, regulatory, and financial risks using grounded yFinance metrics.</p>
