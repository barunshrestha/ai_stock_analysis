...
import { api, ApiError, type DebateAnalysis, type DebateCacheLookup } from "@/lib/api";
...
function geminiErrorMessage(error: unknown): string {
  ...
  return error instanceof Error ? error.message : "Failed to generate debate analysis.";
}
...
function winnerClass(winner: string) {
  if (winner === "bull") return "border-positive/50 bg-positive/5 text-positive";
  if (winner === "bear") return "border-destructive/50 bg-destructive/5 text-destructive";
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        {s?.bull_score != null && (
          <Badge variant="outline" className={cn(scoreClass(s.bull_score))}>
            Bull {s.bull_score}/10
          </Badge>
        )}
        {s?.bear_score != null && (
          <Badge variant="outline" className={cn(scoreClass(s.bear_score))}>
            Bear {s.bear_score}/10
          </Badge>
        )}
        {s?.winner && (
          <Badge variant="outline" className={cn("capitalize", winnerClass(s.winner))}>
            Winner: {s.winner}
          </Badge>
        )}
        {s?.confidence && (
      ...
      {s?.conclusion_one_liner && (
        <p className="text-sm text-muted-foreground">{s.conclusion_one_liner}</p>
      )}
...
/**
 * Issue #13 — bull vs bear debate with DB cache (Use cached vs Regenerate).
 */
export function DebateAnalysisPanel({ symbol }: { symbol: string }) {
      ...
      <CardContent>
        {!displayed && !generateMut.isPending && !generateMut.isError && (
          <div className="space-y-2 text-sm text-muted-foreground">
            <p>
              Two analysts argue the bull and bear cases with data-backed points, then land on a
              balanced conclusion and score each side 1–10.
            </p>
            {cacheQuery.isPending && <p>Checking for a saved debate memo…</p>}
            {hasCache && (
              <p>
                A saved debate is available
