...
def get_earnings_cache(ticker: str, db=Depends(get_db)):
    """Return cached earnings memo if present (200 + cached:false when missing)."""
    symbol = ticker.strip().upper()
    if not symbol:
        raise HTTPException(status_code=422, detail="Ticker is required.")
    row = db.get_ai_analysis_cache(symbol, EARNINGS_ANALYSIS_TYPE)
    if not row:
        return {"cached": False, "symbol": symbol, "source": None}
    return _ai_memo_payload(
        symbol=symbol,
...
def earnings_analysis(req: EarningsRequest, db=Depends(get_db)):
    ...
    try:
        ...
        metrics = {
            **metrics,
            "earnings_context": research_metrics_service.build_earnings_context(symbol),
        }
    except LookupError as exc:
        raise HTTPException(status_code=404, detail=str(exc))
    except ValueError as exc:
    ...
    markdown, structured, error = gemini_service.generate_earnings_memo(metrics)
    if error:
        raise HTTPException(status_code=503, detail=error)
