...
def get_risk_cache(ticker: str, db=Depends(get_db)):
    """Return cached risk memo if present (200 + cached:false when missing)."""
    symbol = ticker.strip().upper()
    if not symbol:
        raise HTTPException(status_code=422, detail="Ticker is required.")
    row = db.get_ai_analysis_cache(symbol, RISK_ANALYSIS_TYPE)
    if not row:
        return {"cached": False, "symbol": symbol, "source": None}
    return _ai_memo_payload(
        symbol=symbol,
...
def risk_analysis(req: RiskRequest, db=Depends(get_db)):
    """Issue #10: risk memo via Gemini (always live; upserts cache)."""
    symbol = req.symbol.strip().upper()
    try:
        metrics = research_metrics_service.build_research_metrics(symbol, req.period)
    except LookupError as exc:
    ...
    markdown, structured, error = gemini_service.generate_risk_memo(metrics)
    if error:
        raise HTTPException(status_code=503, detail=error)
