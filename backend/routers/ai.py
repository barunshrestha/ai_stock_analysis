...
def get_debate_cache(ticker: str, db=Depends(get_db)):
    """Return cached bull vs bear debate if present (200 + cached:false when missing)."""
    symbol = ticker.strip().upper()
    if not symbol:
        raise HTTPException(status_code=422, detail="Ticker is required.")
    row = db.get_ai_analysis_cache(symbol, DEBATE_ANALYSIS_TYPE)
    if not row:
        return {"cached": False, "symbol": symbol, "source": None}
    return _ai_memo_payload(
        symbol=symbol,
...
def debate_analysis(req: DebateRequest, db=Depends(get_db)):
    ...
    markdown, structured, error = gemini_service.generate_debate_memo(metrics)
    if error:
        raise HTTPException(status_code=503, detail=error)
